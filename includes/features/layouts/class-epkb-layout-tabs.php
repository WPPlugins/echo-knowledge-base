...
class EPKB_Layout_Tabs extends EPKB_Layout {
	...
	*/
	public function generate_kb_main_page() {
		...
		$old_tag = isset($_REQUEST['epkb_top_tab']) ? $_REQUEST['epkb_top_tab'] : '';    // TODO remove later
		$new_tag = isset($_REQUEST['top-category']) ? $_REQUEST['top-category'] : '';
		$active_tab = $old_tag . $new_tag;
		$active_cat_id = '0';
		$first_tab = true;
		foreach ( $this->category_seq_data as $category_id => $subcategories ) {
			$category_name = isset( $this->articles_seq_data[ $category_id ][0] ) ?
								$this->articles_seq_data[ $category_id ][0] : __( $this->kb_config['non_existent_category_name'], 'echo-knowledge-base' );
		...
		<div id="epkb-main-page-container" class="epkb-tabs-template">
	...
	private function display_navigation_tabs( $active_cat_id ) {
		...
		if ( $nof_top_categories <= 6 ) {
			...
			$style2 = $this->get_inline_style( 'background-color:: tab_nav_background_color, border-bottom-color:: tab_nav_border_color, border-style: solid, border-bottom-width: 1px' ); ?>

			<section <?php echo $class1 . ' ' . $style1; ?> >

				<ul	class="epkb-nav-tabs epkb-top-categories-list" <?php echo $style2; ?> >
					...
					<?php
						...
						foreach ( $this->category_seq_data as $category_id => $subcategories ) {
							...
							$category_name = isset($this->articles_seq_data[$category_id][0]) ?
												$this->articles_seq_data[$category_id][0] : $this->kb_config['non_existent_category_name'];
	...
	private function display_main_page_content( $active_cat_id ) {
		...
		$class0 = $this->get_css_class('::section_box_shadow');
		...
		foreach ( $this->category_seq_data as $category_id => $box_categories_array ) {
			...
			$class1 = $this->get_css_class( 'epkb_tab_' . ++$ix . ', epkb_top_panel, epkb-tab-panel, ::nof_columns, ::section_font_size, epkb-categories-list ' . $active ); ?>

			<div <?php echo $class1; ?> > <?php

				if ( empty($box_category_list) && ! empty($articles_coming_soon_msg) ) {
					echo '<div class="epkb-articles-coming-soon">' . esc_html__( $articles_coming_soon_msg, 'echo-knowledge-base' ) . '</div>';
				}
				...
				foreach ( $box_category_list as $box_category_id => $box_categories ) {
					...
					$category_name = isset($this->articles_seq_data[$box_category_id][0]) ?
										$this->articles_seq_data[$box_category_id][0] : __( $this->kb_config['non_existent_category_name'], 'echo-knowledge-base' );
					...
					$box_category_data = $this->is_builder_on ? 'data-kb-top-category-id=' . $category_id . ' data-kb-category-id=' . $box_category_id . ' data-kb-type=sub-category ' : '';  			?>
	...
	private function display_level_3_categories( $box_sub_category_list ) {
		$body_style1 = $this->get_inline_style( 'margin-left:: article_list_margin' );		?>
		<ul class="epkb-sub-category" <?php echo $body_style1; ?>> <?php
	...
	private function display_articles_list( $level, $category_id, $sub_category_exists=false ) {
		...
		$sub_category_styles = 'margin-left:: article_list_margin';
		if ( $level == 1 ) {
		...
		<ul <?php echo $style . ' ' . $this->get_inline_style( $sub_category_styles ); ?>> <?php
			...
			if ( empty($articles_list) ) {
				echo '<li class="epkb-articles-coming-soon">' . esc_html__( $articles_coming_soon_msg, 'echo-knowledge-base' ) . '</li>';
			}
			...
			if ( count($articles_list) > $nof_articles_displayed ) { ?>
				<span class="epkb-show-all-articles">
					<span class="epkb-show-text">
						<?php echo esc_html__( $this->kb_config['show_all_articles_msg'], 'echo-knowledge-base' ) . ' ( ' . ( count($articles_list) - $nof_articles_displayed ); ?> )
					</span>
				<span class="epkb-hide-text epkb-hide-elem"><?php esc_html_e( $this->kb_config['collapse_articles_msg'], 'echo-knowledge-base' ); ?></span> <?php
			}  ?>
