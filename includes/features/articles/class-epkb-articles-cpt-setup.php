...
class EPKB_Articles_CPT_Setup {
	...
	public function register_knowledge_base_post_types() {
		...
		foreach ( epkb_get_instance()->kb_config_ojb->get_kb_configs() as $config ) {

			$result = self::register_custom_post_type( $config['id'], $current_id, $config['kb_articles_common_path'] );
			if ( is_wp_error( $result ) ) {
				EPKB_Logging::add_log_var("Could not register custom post type.", $config['id'], $result);
			}
	 ...
	 *
	 * @param int $kb_id is ID of the knowledge base to register post for
	 * @param int|string $current_id
	 * @param string $kb_articles_common_path - assumed to be sanitized already
	 * @return bool|WP_Error
	 */
	public static function register_custom_post_type( $kb_id, $current_id, $kb_articles_common_path = '' ) {

		$kb_post_type = EPKB_KB_Handler::get_post_type( $kb_id );
		$kb_articles_common_path = empty( $kb_articles_common_path ) ?
									EPKB_KB_Handler::get_default_slug( $kb_id ) : $kb_articles_common_path;
		...
		if ( empty($current_id) ) {
			...
			// TODO FUTURE - save the last selected KB ID in plugin settings to use when user switches back to last KB accessed when coming back
			$current_id = min($kb_ids);
		}
		...
		$labels = array(
				'name'               => _x( 'Articles', 'post type general name', 'echo-knowledge-base' ),
	...
	public function output_article_categories( $thelist, $separator=', ', $parents ) {
		...
		if ( defined('WP_ADMIN') || ! $post instanceof WP_Post || ! EPKB_KB_Handler::is_kb_post_type( $post->post_type ) ) {
			return $thelist;
		}
