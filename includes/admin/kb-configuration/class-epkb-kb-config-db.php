...
class EPKB_KB_Config_DB {
	...
	function get_kb_configs( $skip_check=false ) {
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( is_wp_error($kb_id) ) {
				EPKB_Logging::add_log_var("Found invalid kb id", $config['id'], $kb_id);
				continue;
			}
	...
	public function get_value( $setting_name, $kb_id='', $default='' ) {
		...
		if ( is_wp_error( $kb_config ) ) {
			EPKB_Logging::add_log_var( "Could not retrieve KB configuration (15). Settings name: ", $setting_name, $kb_config );
			return $default;
		}
