...
class EPKB_KB_Config_Sequence {
	...
	public function update_articles_sequence( $kb_id, & $new_kb_config ) {
		...
		if ( $is_custom_sequence ) {
			...
			$new_sequence = $this->get_new_sequence( $new_kb_config['kb_main_page_layout'] == EPKB_KB_Config_Layout_Tabs::LAYOUT_NAME  );

			$orig_articles_obj = $this->update_articles_order( $kb_id, $new_sequence );
			if ( $orig_articles_obj === false ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Error occurred' . ' (B32)', 'echo-knowledge-base' ));
			}

			$result = EPKB_Utilities::save_kb_option( $kb_id, EPKB_Articles_Admin::KB_ARTICLES_SEQ_META, $orig_articles_obj->ids_array, true );
			if ( is_wp_error( $result ) ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Error occurred' . ' (B33)', 'echo-knowledge-base' ));
			}
		} else {
	...
	public function update_categories_sequence( $kb_id, & $new_kb_config ) {
		...
		if ( $is_custom_sequence ) {
			...
			if ( $orig_categories_obj === false ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Error occurred' . ' (B24)', 'echo-knowledge-base' ));
			}
			...
			if ( is_wp_error( $result ) ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Error occurred' . ' (B23)', 'echo-knowledge-base' ));
			}
		} else {
	 ...
	 *
	 * @param $is_layout_top_categories
	 * @return string
	 */
	public function get_new_sequence( $is_layout_top_categories ) {

		$new_sequence = isset($_REQUEST['epkb_new_sequence']) ? $_REQUEST['epkb_new_sequence'] : array();
		...
		if ( $is_layout_top_categories ) {

			$top_cat_sequence = isset($_REQUEST['top_cat_sequence']) ? $_REQUEST['top_cat_sequence'] : array();
			foreach( $top_cat_sequence as $top_category_id ) {
				...
				foreach( $new_sequence as $key => $item ) {
					...
					if ( $level == 'category' && $found_category ) {
						break;
					}
	...
	public function update_categories_order( $kb_id, $new_sequence, $orig_categories_obj=null ) {
		...
		$new_sequence = $this->normalize_new_sequence( $new_sequence );
		if ( empty($new_sequence) ) {
			return false;
		}
		...
		// TODO check nothing is missing in the new structure
	 ...
	 * TODO: log why false returned (in all methods)
	 */
	...
	/**
	 * Ensure that received sequence has correct format.
	 *
	 * @param $new_sequence
	 * @return array|false - false to indicate an error
	 */
	public function normalize_new_sequence( $new_sequence ) {
