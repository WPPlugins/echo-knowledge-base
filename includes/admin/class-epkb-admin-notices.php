...
class EPKB_Admin_Notices {
	...
	public function show_admin_notices() {
		...
		$param = '';
		if ( ! empty($_GET['epkb_notice_param']) ) {
			$param = ' ' . sanitize_text_field( $_GET['epkb_notice_param'] );
		}
		...
		echo  EPKB_Utilities::get_html_message_box( $message, '', $class );
	}
	...
	function dismiss_admin_notices() {
		if ( ! isset( $_GET['epkb_dismiss_notice_nonce'] ) || ! wp_verify_nonce( $_GET['epkb_dismiss_notice_nonce'], 'epkb_dismiss_notice') ) {

			wp_die( __( 'Security check failed', 'echo-knowledge-base' ), __( 'Error', 'echo-knowledge-base' ), array( 'response' => 403 ) );
		}

		if ( isset( $_GET['epkb_admin_notice'] ) ) {
			update_user_meta( get_current_user_id(), '_epkb_' . $_GET['epkb_admin_notice'] . '_dismissed', 1 );
