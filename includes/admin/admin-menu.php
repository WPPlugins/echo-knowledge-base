...
function epkb_add_plugin_menus() {
	...
	add_submenu_page( 'edit.php?post_type=' . $post_type_name,
					__( 'Configuration - Echo Knowledge Base', 'echo-knowledge-base' ), __( 'Configuration', 'echo-knowledge-base' ), 'manage_options', 'epkb-kb-configuration', array( new EPKB_KB_Menu_Configuration, 'display_kb_config_page') );
	add_submenu_page( 'edit.php?post_type=' . $post_type_name,
					__( 'Plugin Info - Echo Knowledge Base', 'echo-knowledge-base' ), __( 'Plugin Info', 'echo-knowledge-base' ), 'manage_options', 'epkb-plugin-settings', array( new EPKB_Settings_Page(), 'display_plugin_settings_page' ) );
	add_submenu_page( 'edit.php?post_type=' . $post_type_name,
					__( 'Add-ons - Echo Knowledge Base', 'echo-knowledge-base' ), __( 'Add-ons', 'echo-knowledge-base' ), 'manage_options', 'epkb-add-ons', array( new EPKB_Add_Ons_Page, 'display_add_ons_page') );
}
...
function epkb_add_page_tabs() {
	...
	switch ( $screen_id ) {
		...
		case 'EKB_SCREEN_page_epkb-kb-configuration':
			$tab_url_base = 'edit.php?post_type=*&page=epkb-kb-configuration';
			return;
	...
	<div id="ekb_core_top_heading">
		<ul class="tab_navigation">  			<?php
			...
			foreach ( $active_kb_configs as $one_kb_config ) {
				...
				// output KB tab
				$kb_name = isset($one_kb_config['name']) ? $one_kb_config['name'] : ( isset($one_kb_config['kb_name']) ? $one_kb_config['kb_name'] : __( 'Knowledge Base', 'echo-knowledge-base' ) );  // TODO revert
		...
		// show KB 'create' button if applied
		if ( in_array($screen_id, array('edit-EKB_SCREEN', 'edit-EKB_SCREEN_category') )) {
			echo apply_filters( 'epkb_add_kb_button', '' );
		}

		$kb_config = epkb_get_instance()->kb_config_ojb->get_current_kb_configuration();	// for each KB type create a tab
		echo EPKB_KB_Handler::get_kb_status_line( $kb_config ); ?>
