<?php
/**
 * The template for displaying Breadcrumb for single KB Article.
 *
...
$class0 = EPKB_Utilities::get_css_class('::section_box_shadow', $kb_config );
$breadcrumb_style1 = EPKB_Utilities::get_inline_style('
	                   padding-top::    breadcrumb_padding_top, padding-right:: breadcrumb_padding_right,
				       padding-bottom:: breadcrumb_padding_bottom, padding-left:: breadcrumb_padding_left', $kb_config );
$breadcrumb_style2 = EPKB_Utilities::get_inline_style( 'color:: breadcrumb_text_color', $kb_config );         ?>

<div class="epkb-breadcrumb" <?php echo $breadcrumb_style1; ?>>
	<div class="epkb-breadcrumb-label"><?php echo esc_html($kb_config['breadcrumb_description_text']); ?></div>
	<ul class="epkb-breadcrumb-nav">       <?php

    $ix = 0;
	foreach ( $breadcrumb as $text => $link ) {

		echo '<li>';
		echo '	<span class="epkb-breadcrumb-link">';

        $ix++;
        $text = empty($text) && $ix == 1 ? __( 'KB Home', 'echo-knowledge-base' ) : $text;
        $text = empty($text) && $ix > 1 ? __( 'Link ' . ($ix - 1), 'echo-knowledge-base' ) : $text;
        ...
        if ( $ix < sizeof($breadcrumb) ) {
            ...
            echo '<span class="epkb-breadcrumb-link-icon ' . esc_html($kb_config['breadcrumb_icon_separator']) . '"></span>';
        } else {
            echo '<span ' . $breadcrumb_style2 . ' >' . esc_html( $text ) . '</span>';
        }
