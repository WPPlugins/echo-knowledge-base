...
$tags_list = get_the_term_list( $article->ID, $taxonomy_name, '<ul class="epkb-tag-list"><li ' . $tag_style2 . '>', '</li><li '.$tag_style2.'>', '</li></ul>' );

if ( ! is_wp_error( $tags_list ) && $tags_list ) {
	echo '<div class="epkb-tag-container" ' . $tag_style1 . ' >';
	printf( '<span class="epkb-tag-description">%1$s </span>%2$s', _x( 'Tags', 'Text used in front of tag names.', 'echo-knowledge-base' ) . ': ',	$tags_list );
	echo '</div>';
}
